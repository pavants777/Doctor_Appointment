  ...
  const getAvailableSlots = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      while (currentDate < endTime) {
        ...
        timeSlots.push({ datetime: new Date(currentDate), time: formattedTime, available: isSlotAvailable });
        ...
        <div className="flex items-center gap-3 w-full overflow-x-scroll mt-4">
          {docSlots[slotIndex]?.map((item, index) => (
            <button
              ...
              className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${!item.available ? 'bg-gray-300 text-gray-500' : item.time === slotTime ? 'bg-primary text-white' : 'text-gray-400 border border-gray-300'}`}
            >
